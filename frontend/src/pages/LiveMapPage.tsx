...
export default function LiveMapPage() {
  ...
  const [currentLocation, setCurrentLocation] = useState<[number, number] | null>(null); // [lng, lat]  const [routeGeoJSON, setRouteGeoJSON] = useState<any>(null);
    ...
    const fetchData = async () => {
      try {
        ...
        if (liveRes.data.success && liveRes.data.data?.currentLocation) {
          ...
          setCurrentLocation(loc);          setSpeed(speed || 0);
  ...
  return (
    <div className="relative w-full h-screen bg-slate-900 overflow-hidden">
      ...
      {/* Bottom Dashboard */}      {currentLocation && (
