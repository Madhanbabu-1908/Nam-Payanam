...
export default function Dashboard() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (tripRes.data.success && tripRes.data.data) {
          ...
          setStats({ spent: totalSpent, members: 1, daysLeft }); // Members count mock for now, fetch from members endpoint in prod
        }
      } catch (err) {
  ...
  const menuItems = [
    ...
    { title: 'Live Map', icon: Navigation, color: 'text-purple-700 dark:text-purple-300', bg: 'bg-purple-50 dark:bg-purple-900/30', path: 'live', desc: 'Real-time tracking' },
    { title: 'Members', icon: Users, color: 'text-pink-700 dark:text-pink-300', bg: 'bg-pink-50 dark:bg-pink-900/30', path: 'members', desc: 'Group info' },
    { title: 'Settings', icon: Settings, color: 'text-slate-700 dark:text-slate-300', bg: 'bg-slate-100 dark:bg-slate-800', path: 'settings', desc: 'Options' },
  ];
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {/* ✅ FIX: Display human-readable destination name, fallback to coords if missing */}
              <p className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1">                <MapPin size={12} /> {trip.destination || `${trip.destination_lat?.toFixed(4)}, ${trip.destination_lng?.toFixed(4)}`}
              </p>
            </div>
          </div>
