    ...
    <div className="flex flex-col pb-28 animate-fade-in">
      ...
      <div className="px-4 pt-3 space-y-3">
        {filtered.length === 0 ? (
          ...
          filtered.map(b => (
            <BreakCard key={b.id} breakEntry={b}
              expenses={expenses.filter(e => e.break_id===b.id)}
              session={session} trip={trip} members={members}
      ...
      <BreakSheet isOpen={showAdd||!!editBreak} onClose={() => { setShowAdd(false); setEditBreak(null); }}
        trip={trip} members={members} session={session} editData={editBreak}/>
    </div>
  );
}

function BreakCard({ breakEntry: b, expenses, session, trip, members, onEdit }) {
    ...
    <div className="card p-4 space-y-3">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between">
            ...
            {canEdit && (
              <button onClick={onEdit} className="btn-icon bg-slate-100 w-8 h-8 flex-shrink-0 ml-2">
      ...
      {!b.checkout_time && canEdit && (
        <button onClick={checkout} disabled={checkingOut}
          ...
          {checkingOut ? <Spinner size="sm" color="indigo"/> : '✅'} Check Out Now
        </button>
      )}
...
function BreakSheet({ isOpen, onClose, trip, members, session, editData }) {
  const [form, setForm] = useState({ stopName:'', breakType:'rest', customType:'', description:'', checkinTime:'', checkoutTime:'', dayNumber:0 });
  ...
  async function save() {
    ...
    try {
      const payload = {
        tripId: trip.id, dayNumber: form.dayNumber, addedByNickname: session?.nickname,
        stopName: form.stopName.trim(), breakType: form.breakType,
        customType: form.customType, description: form.description,
        checkinTime: form.checkinTime || new Date().toISOString(),
        checkoutTime: form.checkoutTime || null,
      };
      if (editData) { await breakAPI.update(editData.id, payload); toast.success('Break updated!'); }
      else { await breakAPI.add(payload); toast.success('Break logged! ☕'); }
      onClose();
    ...
    <BottomSheet isOpen={isOpen} onClose={onClose} title={editData ? 'Edit Break' : 'Log a Break'}>
      <div className="space-y-4 pb-4">
        <div><label className="label">Stop Name *</label>
          <input className="input" placeholder="e.g. Dhaba near Krishnagiri" value={form.stopName} onChange={e=>upd('stopName',e.target.value)}/></div>
        <div><label className="label">Break Type</label>
        ...
        {form.breakType==='other' && (
          <div><label className="label">Custom Type</label>
            <input className="input" placeholder="Describe the stop type..." value={form.customType} onChange={e=>upd('customType',e.target.value)}/></div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div><label className="label">Check-out Time</label>
            <input type="datetime-local" className="input text-sm" value={form.checkoutTime} onChange={e=>upd('checkoutTime',e.target.value)}/></div>
        </div>
        <div><label className="label">Description (optional)</label>
          <textarea className="input resize-none text-sm" rows={2} placeholder="Notes about this stop..." value={form.description} onChange={e=>upd('description',e.target.value)}/></div>
