...
export const EmptyStateItinerary: React.FC<Props> = ({ onAddLocation, onUseTemplate }) => {
  return (
    <div className="flex flex-col items-center justify-center p-8 text-center bg-white rounded-xl border-2 border-dashed border-gray-300 m-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 w-full max-w-md">
        <button onClick={onAddLocation} className="flex-1 bg-indigo-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-indigo-700 flex items-center justify-center">
          <Plus className="h-4 w-4 mr-2" /> Add Location
        </button>
        <button onClick={onUseTemplate} className="flex-1 bg-white text-indigo-600 border border-indigo-600 px-4 py-2 rounded-lg font-medium hover:bg-indigo-50 flex items-center justify-center">
