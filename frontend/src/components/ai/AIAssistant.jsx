...
export default function AIAssistant({ trip, session, lang='en' }) {
  ...
  return (
    <div className="flex flex-col animate-fade-in" style={{ height:'calc(100svh - 200px)' }}>

      {/* Header */}
      <div className="px-4 pt-3 pb-2">
        <div className="bg-gradient-to-r from-indigo-700 to-indigo-900 rounded-2xl p-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="text-indigo-300 text-xs">Groq · LLaMA 3.3 · Context-aware</div>
          </div>
      ...
      <div className="px-4 pb-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          {ACTIONS.map(a=>(
            <button key={a.id} onClick={()=>send(a.prompt, a.id)}
              disabled={!!loading}
              className="ai-action text-left disabled:opacity-50">
              <span className="text-xl flex-shrink-0">{a.icon}</span>
              <div className="min-w-0">
                <div className="font-bold text-slate-800 text-xs truncate">{lang==='ta'?a.ta:a.label}</div>
                {loading===a.id && <div className="flex gap-0.5 mt-1">{[0,1,2].map(i=>(
      ...
      <div className="flex-1 overflow-y-auto px-4 py-2 space-y-3 scrollbar-hide">
        {messages.map((msg,i)=>(
          <div key={i} className={`flex ${msg.role==='user'?'justify-end':'justify-start'} animate-fade-in`}>
            {msg.role==='assistant' && (
              <div className="w-8 h-8 bg-indigo-600 rounded-xl flex items-center justify-center text-sm mr-2 mt-0.5 flex-shrink-0">🤖</div>
            )}
      ...
      {/* Input */}
      <div className="px-4 pb-safe pt-2 border-t border-slate-100">
        <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-2xl px-3 py-2 focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
          ...
          <button onClick={()=>send()} disabled={!input.trim()||!!loading}
            className="w-9 h-9 bg-indigo-600 text-white rounded-xl flex items-center justify-center disabled:opacity-40 active:scale-95 flex-shrink-0">
